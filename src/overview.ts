...
interface Page {index:number;images:ImageRecord[];canvas?:HTMLCanvasElement;ready:Set<number>;queued:boolean;loading:boolean;complete:boolean;listeners:Set<()=>void>}
...
export class OverviewAtlas {
 ...
 private newPage(index:number,images:ImageRecord[]):Page{return{index,images,ready:new Set(),queued:false,loading:false,complete:false,listeners:new Set()};}
 ...
 private async load(page:Page):Promise<void>{
  try{
   ...
   page.complete=true;this.notify(page);
